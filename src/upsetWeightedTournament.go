...
func determinWinner(team1 Team, team2 Team) Team {
  ...
  if len(os.Args) >= 5 {
    if os.Args[4] == `true` {
      fmt.Printf(`Rand(%v, %v) = %v <= %v   `, 1, favorite.Seed*coefficient + underdog.Seed*coefficient, outcome, favorite.Seed*coefficient+upsetCoefficient+underdog.NumUpsets)
    }
  }
  if outcome <= favorite.Seed*coefficient+upsetCoefficient+underdog.NumUpsets { // the larger this second value is the higher chance an underdog wins
    // underdog wins
    underdog.NumUpsets = underdog.NumUpsets + ((underdog.Seed - favorite.Seed) * upsetterMultiplier)
    return underdog
  } else {
    return favorite
  }
